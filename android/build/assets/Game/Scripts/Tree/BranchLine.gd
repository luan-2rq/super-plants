	...
	if branch_data.collectable_holder_count > 0:
		if branch_data.collectables_holders_data.size() <= 0:
			for i in range(branch_data.collectable_holder_count):
				...
				var collectable_holder = tree_config.collectable_prefab.instantiate()
...
func add_point_(position : Vector2, index : int = -1):
	...
	raycast.target_position = (points[-1] - points[-2]).normalized() * 10 if points.size() > 2 else points[-1].normalized() * 10
...
func grow(length) -> BranchPointsResult:
	...
	raycast.target_position = (self.points[-1] - self.points[-2]).normalized() * 10 if self.points.size() > 2 else self.points[-1].normalized() * 10
	...
	#var line_poly = Geometry.offset_polygon(self.points, width/2)
...
func generate_points_random_direction(angle_range : float, initial_direction : Vector2) -> PackedVector2Array:
...
func trim_points(points : PackedVector2Array, bounds : Bounds) -> BranchPointsResult:
...
func expand(point_a : Vector2, point_b : Vector2, delta : int) -> PackedVector2Array:
...
func random_lengths(n: int, min_length: float, max_length: float, total_length: float) -> Array:
	...
	for i in range(n):
		var random_value = randf_range(min_length, max_length)
