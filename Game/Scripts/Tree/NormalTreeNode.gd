		...
		if tree == null:
			...
			while i < plant_data.active_branchs.size():
				if plant_data.active_branchs[i].filled_percentage < 1:
					...
					if abs(result.points[-1].y + plant_data.active_branchs[i].instance.global_position.y - (self.get_parent().rect_global_position.y + self.get_parent().rect_size.y)) > cur_max_point.y:
						cur_max_point = (result.points[-1] + plant_data.active_branchs[i].instance.global_position - (self.get_parent().rect_global_position + self.get_parent().rect_size)).abs()
			...
			while i < branchs_to_spawn_count:
				...
				if branch_parent.filled_percentage >= branch_to_be_spawned.position_in_parent:
					...
					if abs(result.points[-1].y + branch_to_be_spawned.instance.global_position.y - (self.get_parent().rect_global_position.y + self.get_parent().rect_size.y)) > cur_max_point.y:
						cur_max_point = (result.points[-1] + branch_to_be_spawned.instance.global_position - (self.get_parent().rect_global_position + self.get_parent().rect_size)).abs()
					...
					plant_data.branchs_to_be_spawned.remove(i)
