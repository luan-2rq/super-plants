...
#Config
export var tree_config : Resource
...
func grow_tree():
	var cur_branch
	if cur_depth <= tree_config.max_depth:
		#1: Gera primeiro branch e coloca os filhos na filha de spawn
		if tree == null:
			tree = BranchData.new(null, cur_depth, 0, tree_config.initial_branch_length, tree_config.n_points_per_branch, tree_config.bake_interval, 0)
			tree.instance = BranchLine.new(tree, tree_config.stem_color, tree_config.leaf_texture, tree_config.leaf_color)
			tree.instance.width = 5
			tree.global_pos = self.global_position
			active_branchs.append(tree)
			add_child(tree.instance)
			
			tree.instance.grow(tree_config.step_length, tree_config.branch_angle_range, tree_config.initial_branch_direction, tree_config.bounds)
			cur_depth += 1
			for i in range(tree_config.initial_branch_count):
				var position_in_parent = Random.range_float(0, 1)
				cur_branch = BranchData.new(tree, cur_depth, position_in_parent, tree_config.max_branch_length, tree_config.n_points_per_branch, tree_config.bake_interval, Random.range_int(0,  tree_config.max_leaf_count))
			...
			while i < active_branchs.size():
				if active_branchs[i].filled_percentage < 1:
					var result : BranchPointsResult = active_branchs[i].instance.grow(tree_config.step_length, tree_config.branch_angle_range, tree_config.initial_branch_direction, tree_config.bounds)
					if (result.points[-1].y * tree_config.initial_branch_direction.y + active_branchs[i].instance.global_position.y - tree.instance.global_position.y) * tree_config.initial_branch_direction.y > cur_max_point.y:
						cur_max_point = ((result.points[-1] * tree_config.initial_branch_direction.y) + active_branchs[i].instance.global_position - tree.instance.global_position) * tree_config.initial_branch_direction.y
			...
			while i < branchs_to_spawn_count:
				...
				if branch_parent.filled_percentage >= branch_to_be_spawned.position_in_parent:
					...
					branch_to_be_spawned.instance = BranchLine.new(branch_to_be_spawned, tree_config.stem_color, tree_config.leaf_texture, tree_config.leaf_color)
					...
					var result : BranchPointsResult = branch_to_be_spawned.instance.grow(tree_config.step_length, tree_config.branch_angle_range, tree_config.initial_branch_direction, tree_config.bounds)
					if (result.points[-1].y * tree_config.initial_branch_direction.y + branch_to_be_spawned.instance.global_position.y - tree.instance.global_position.y) * tree_config.initial_branch_direction.y > cur_max_point.y:
						cur_max_point = ((result.points[-1] * tree_config.initial_branch_direction.y)+ branch_to_be_spawned.instance.global_position - tree.instance.global_position) * tree_config.initial_branch_direction.y
						
					#3.2: Coloca na fila de spawn os filhos da branch spawnada
					var branch_count = Random.range_int(1, tree_config.max_branch_count)
					for j in range(branch_count):
						if cur_depth == branch_to_be_spawned.depth:
							cur_depth += 1
						var position_in_parent = Random.range_float(0, 1)
						cur_branch = BranchData.new(branch_to_be_spawned, cur_depth, position_in_parent, tree_config.max_branch_length, tree_config.n_points_per_branch, tree_config.bake_interval, Random.range_int(0, tree_config.max_leaf_count))
				...
				i+=1
	Events.emit_signal("on_grow", tree_config.tree_type, cur_max_point)
